...
import 'package:tagxi_super_user/pages/loadingPage/loadingpage.dart';
import 'package:tagxi_super_user/pages/onTripPage/booking_confirmation.dart';
import 'package:tagxi_super_user/pages/login/get_started.dart';
import 'package:tagxi_super_user/pages/login/login.dart';
import 'package:tagxi_super_user/pages/onTripPage/map_page.dart';
import 'package:tagxi_super_user/pages/onTripPage/review_page.dart';
import 'package:tagxi_super_user/pages/referralcode/referral_code.dart';
...
//base url
String url =
    //  'http://192.168.1.67:8000/'; //add '/' at the end of url as 'https://yourwebsite.com/'

    // 'http://217.76.58.38:30016/'; //add '/' at the end of url as 'https://yourwebsite.com/'

    //'https://gochap.app/';
    'https://gochap-acap.blackcoast-1ee958b7.westeurope.azurecontainerapps.io/';
String urlProvider = 'http://31.220.95.109:30030/';
//String mapkey = 'AIzaSyDNvBt4rb284qJQVbkCjxKJ2zS0CdHLR28';
...
otpCall() async {
  ...
  try {
    ...
    //var otp = await FirebaseDatabase.instance.ref('call_FB_OTP').get();
    result = otp;
  } catch (e) {
    if (e is SocketException) {
      internet = false;
...
getGoodsList() async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      goodsTypeList = chooseGoods[choosenLanguage];
      valueNotifierBook.incrementNotifier();
      result = 'success';
    } else {
      debugPrint(response.body);
...
mobileMoneyToWallet() async {
  ...
  try {
    await getUserDetails();
    if (userDetails['active'] == 1) {
      code = userDetails['country_id'];
      await getCountryCode();
      pays = countries.where((element) => element['id'] == code).first;
      var response =
          await http.post(Uri.parse('${urlProvider}api/Transactionsts/Payment'),
              headers: {
                ...
                'environnment': xEnv,
                'Content-Type': 'application/json',
                'x-token': 'Bearer ${bearerToken[0].token}',
              },
              body: jsonEncode({
                "amount": addMoney,
                "msisdn": "${userDetails['mobile']}",
                "otp": otp,
                "name": nameProvider,
                "type": 'mobile_money',
                "reference": Uuid().v1(),
